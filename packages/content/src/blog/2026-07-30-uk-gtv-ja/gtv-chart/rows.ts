...
const submittedIndex = starPoints.findLastIndex((point) => point.at <= submittedAt);

/** Star history up to submission: what the application was actually assessed on. */
export const starsAssessed = starPoints.slice(0, submittedIndex + 1);

/** Star history after submission, sharing a point so the two lines meet. */
export const starsAfterSubmission = starPoints.slice(submittedIndex);
