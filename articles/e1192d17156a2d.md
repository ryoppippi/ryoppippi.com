...
チームメンバーが Svelte に興味があったことが採用の決め手となりました。

今回は build は SSG (Static Site Generation) を採用し、SSR 等の機能は使用していません。
そのため、後述のように、 Cloudflare Pages では実質無料で運用することができます。

https://kit.svelte.jp/docs/adapter-static

### Svelte5
Svelte5 は現時点ではまだ Preview 版ですが十分実用的でした。
今回は `runes` や `snippets` など Svelte5 で導入された新機能をがっつり使ってみましたが、とても書き味が良かったです。

`runes` はコンポーネントの引数の受け渡し、Reactivity のための実装などありとあらゆる場面で使用しましたが、型安全性も増し、冗長な記述も減り、とても書き味が良いものでした。

`snippets` はSvelte のファイル内に繰り返し使うコードをまとめて置くことができる機能です。
Svelte4までは、同じような実装を使い回すには別ファイルにコンポーネントとして切り出す必要がありましたが、Svelte5 ではファイル内にまとめて置くことができるようになりました。
そのため、例えば responsive design などで同じようなコードを繰り返し書く場合に簡潔に実装することができました。
...
`enhanced-img` は、screen サイズに応じて適切な画像が選択されるように複数サイズの画像を自動で生成したり、`png` 等の画像を `webp` や `avif` に変換してくれる機能を提供してくれます。
こちらも preview 版であり、１ヶ月前の breaking change の影響で import 文でtype error が発生するなどの不具合がありましたが、概ね問題なく使用することができました。

自動で画像の最適化を行ってくれるのはやはり便利ですね。
こちらもパフォーマンス向上に大きく寄与してくれました。

### favicon/metadata の自動生成

LP のサイトは favicon や metadata の設定が重要です。
ある程度は手作業で問題ないものの、様々なサイズの favicon を用意するなど一部手作業では面倒な部分もあります。
そこで、favicon や metadata の生成を自動化するために `favicons` を使用しました。

https://github.com/itgalaxy/favicons

このライブラリは、設定に応じて metadata や 様々なサイズの favicon を生成してくれます。
今回は `favicons` を使用してビルド時に favicon や metadata を生成するための vite plugin を作成、使用しました。
...
上のブログでも紹介されているように、BudouXは非常に便利なツールです。
しかし、そのまま実装すると約7KBのbundle cost および runtime cost が発生します。

このランタイムコストを避けるため、今回の LP のために を作成しました。

そこで、この LP の制作に合わせて、新たな [ Svelte Preprocessor ](https://kit.svelte.jp/docs/integrations#preprocessors) を作成しました。
これにより、runtime cost を抑えつつ BudouX の機能を活用できます。

https://github.com/ryoppippi/svelte-preprocess-budoux

使い方は簡単です：
1. まず、`svelte.config.js` にプリプロセッサを設定します。
...
*windowの幅が変わっても単語が途中で途切れることなく適切に改行されている*

この方法により、ランタイムのオーバーヘッドを増やすことなく、テキストを適切に改行できるようになりました。
添付の画像からわかるように、ウィンドウの幅が変わっても、テキストが自然に改行されています。
いい感じですね！

### URL Validation on build time

このプロジェクトでは、URL が正しいものであるかをbuild時に検証しています。

SvelteKit には `building` や `dev` といった変数があり、これらが `true` の時にのみ実行されるコードを書くことができます。
今回は、`ensureURL` という関数を作成し、dev 環境および build 時のみ validation logic を実行するようにしました。
...
これにより、bundle cost および runtime cost をゼロにしながらも、見た目は公式のものと変わらない投稿カードを埋め込むことができます。
...
https://github.com/vim-jp-radio/LP/blob/2f9c28b4a4de4cd7be8db61a78ae04fd5bd08a03/src/lib/utils/runes.svelte.ts
...
TailwindCSS では全ての style を class に書くので、長くなりがちです。また、style の種類ごとに分割したい場合にも対応できません。
UnoCSS では、属性として style を書くことができるため、コードの見通しが良くなります。
...
これらを定義することで、繰り返し登場するような style を簡潔に書くことができます。
...
UnoCSS Inspector を用いると、ページごと、Component ごとにどの style が適用されているかを簡単に確認することができます。
このツールのおかげで不要な style を削除することができ、CSS の bundle size を削減することができました。
また、style がうまく適用されない時の debug にも大いに役立ちました。

個人的には TailwindCSS の上位互換のように思えました。
今後も積極的に採用していきたいと思います。


:::message

今回、[属性に直接styleを書く記法](https://unocss.dev/presets/attributify) を採用しましたが、1つだけ tip があります。
...
このコードでも大抵の場合は問題ありませんし、Document でも採用されている記法です。
しかし、将来的にこの記法が問題になる可能性があります。
例えば、`mb` のような属性が規格として追加された場合どうでしょうか。既存のコードが意図しない挙動をする可能性がありますね。
...
前述のように、SvelteKit で SSG を採用しているため、Cloudflare Wokers を一切使用していません。
そのため、基本無料で運用することができます。

### Redirect

今回のプロジェクトでは特定の Path に対して redirect を設定し、短縮 URL のように使用しています。
例えば [`https://vim-jp-radio.com/apple`](https://vim-jp-radio.com/apple) にアクセスすると、Apple Podcast のページにリダイレクトされます。

実は、Cloudflare Pages では `_redirects` ファイルを一緒に deploy することで、redirect を設定することができます。
これにより、 workers を使用することなく、redirect を設定することができます。

https://developers.cloudflare.com/pages/configuration/redirects/

しかし、この `_redirects` ファイルの内容を手動で管理するのは面倒です。
また、dev 環境(vite) は標準では `_redirects` ファイルを認識しないため、手元で redirect を確認することができません。
...
- dev や preview 時の vite server で動作する middleware を提供し、local で redirect を確認することができる

これにより `_redirects` ファイルの管理を自動化し、local で redirect を確認できるようになりました。
...
ぜひ探してみてください。
そして、見つけたら X や BlueSky で [ハッシュタグ `#vimjpradio`](http://x.com/share?hashtags=vimjpradio&url=vim-jp-radio.com) をつけて投稿してみてください。

# おわりに

vim-jp ラジオの LP を制作しました。

我々自身もvim-jp ラジオをリスナーとしてもとても楽しみにしています。
