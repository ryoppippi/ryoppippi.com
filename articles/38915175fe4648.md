---
title: "なぜNeovimmerの僕はマルチカーソルを必要としないか"
emoji: "🤹"
...
0. `*`でカーソル下の単語を検索
1. `''`で元の位置に戻る(詳しくは[''もしくは``で直前のジャンプ位置に戻る](https://vim-jp.org/vimdoc-ja/motion.html#''))参照)
2. `cgn`で単語を編集
3. 'n'で次の検索結果へ移動
4. `.`で単語の置換を繰り返す
...
2. `<C-r><C-r>"`でヤンクした内容をペースト([<C-r><C-r>](https://vim-jp.org/vimdoc-ja/insert.html#i_CTRL-R_CTRL-R)参照)
