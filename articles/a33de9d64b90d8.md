...
topics: ["vim", "neovim", "cli", "aquaclivm", "gh"]
...
私はmacOSで使うツールの管理に以下のパッケージマネージャたちを使っています。

- 汎用的なパッケージマネージャ
  - [aqua](https://aquaproj.github.io/) - YAMLでDeclaretiveにツールを管理できる。極力これで管理したい
  - [Homebrew](https://brew.sh/) - 有名なパッケージマネージャ。macOSのデファクトスタンダード。Aquaで管理できないツールを管理する
- 言語ごとのパッケージマネージャ
  ...
  - [cargo](https://doc.rust-lang.org/cargo/) - Rustのパッケージマネージャ。Rustのツールを管理するのに使う
  - [zigup](https://github.com/marler8997/zigup) - Zigのバージョン管理ツール。Zigのバージョンを切り替えるのに使う
- ツールごとのパッケージマネージャ
  - [fisher](https://github.com/jorgebucaran/fisher) - fishのプラグインマネージャ。fishのプラグインを管理するのに使う
  - [lazy.nvim](https://github.com/folke/lazy.nvim) - Neovimのプラグインマネージャ。Neovimのプラグインを管理するのに使う

結構多いですね。
ツールは多岐にわたるので、それぞれのツールに合ったパッケージマネージャを使っています。
...
自分がdotfilesで使用するパッケージマネージャに求める条件は以下の通りです。
- パッケージマネージャのインストール、アンインストールが簡単
- パッケージマネージャの設定をDeclaretiveに記述できる
- パッケージマネージャが管理するツールの設定やバージョンをDeclaretiveに記述できる
- Lockfileでバージョン、差分を管理できる
- パッケージマネージャが管理するツールのバージョンを切り替えることができる/簡単に戻すことができる
- 設定量は多過ぎず少な過ぎず
- 暗黙の動作が少ない
...
これらを条件を考えた時に、汎用のパッケージマネージャにおいては、Aquaが一番条件を満たしていると思います。

https://aquaproj.github.io/

AquaはYAMLでDeclaretiveにツールを管理でき、またRenovateを併用すれば最新版の確認、更新も簡単にできます。
...
とても良いパッケージマネージャではあるので、ぜひ皆さんチェックしてみてください。

# 解: lazy.nvimを悪用する
というわけで、自分が求める条件を満たすパッケージマネージャがなかなか見つかりません。
...
- パッケージマネージャーの設定をDeclaretiveに記述できる →  lua で記述できる
- パッケージマネージャーが管理するツールの設定やバージョンをDeclaretiveに記述できる →  lua で記述できる
