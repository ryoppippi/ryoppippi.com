...
# TL;DR
- この度、`unplugin-typia`というライブラリを作りました
...
ZodやValibotなどの既存のValidation Libraryは、TSのライブラリとして用意されたDSL（ドメイン固有言語）で assertion を定義し、そこからTypeScriptの型を生成(推論)することで型安全を担保しています。
これでは既存の型に対するバリデーション関数を素朴な方法で用意できず、DSLを覚えて、一からバリデーション関数を作りなおす必要があります。
...
- 高速: `Typia`は既存のバリデーションライブラリに比べて非常に高速です。「zodの1500倍速い」とも言われています^[https://zenn.dev/hr20k_/articles/3ecde4239668b2#%E9%80%9F%E5%BA%A6%E3%81%AE%E6%AF%94%E8%BC%83]。
...
この点は、他のバリデーションライブラリとの大きな差別化要因であり、`Typia`の導入障壁を大幅に下げています。
