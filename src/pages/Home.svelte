<script lang="ts">
	import ProfileImage from '@/components/ProfileImage/index.svelte';

	import { SITE_ORIGIN } from '@/config/site.ts';

	const socials = [
		['icon-[line-md--github-loop]', 'GitHub profile', '/github'],
		['icon-[ph--git-pull-request-duotone]', 'Recent pull requests', '/pr'],
		['icon-[line-md--linkedin]', 'LinkedIn profile', '/linkedin'],
		['icon-[line-md--twitter]', 'Twitter profile', '/twitter'],
		['icon-[simple-icons--bluesky]', 'Bluesky profile', '/bsky'],
		['icon-[ri--youtube-line]', 'YouTube channel', '/youtube'],
	] as const satisfies readonly (readonly [icon: string, label: string, href: string])[];
</script>

<article class="homeIntro">
	<div class="homeProfileStage" data-home-profile>
		<ProfileImage class="homeProfileImage" fetchpriority="high"></ProfileImage>
	</div>
	<div class="homeHeading">
		<h1 class="homeTitle" style="view-transition-name:title-ryoppippi">
			<span class="homeHandle">@ryoppippi</span>{' '}<span class="homeRole">Engineer</span>
		</h1>
	</div>
</article>
<div class="homeSocialsFrame">
	<article class="homeSocials">
		{#each socials as [icon, label, href]}
			<div class="homeSocialCard">
				<a
					aria-label={label}
					href={`${SITE_ORIGIN}${href}`}
					rel="noopener noreferrer"
					target="_blank"><span class={`${icon} homeSocialIcon`} aria-hidden="true"></span></a
				>
			</div>
		{/each}
	</article>
</div>

<style>
	.homeIntro {
		display: grid;
		width: 100%;
		place-content: center;
		place-items: center;
		margin: 2rem auto 0;
	}

	:global(body[data-page-style='.']) {
		font-family: var(--font-sans);
	}

	.homeProfileStage {
		width: 100%;
	}

	.homeProfileStage :global(.homeProfileImage) {
		display: block;
		width: 50%;
		max-width: 100%;
		aspect-ratio: 1;
		margin-inline: auto;
		border-radius: 9999px;
		object-fit: contain;
		view-transition-name: name---profile;
	}

	.homeHeading {
		margin-top: 2rem;
	}

	.homeTitle {
		margin-bottom: 1.5rem;
		text-align: center;
		font-size: 2.25rem;
		line-height: 2.5rem;
		font-weight: 700;
	}

	.homeHandle {
		display: block;
	}

	.homeRole {
		text-align: center;
		font-size: 1.25rem;
		line-height: 1.75rem;
		font-weight: 500;
		color: var(--color-text-700);
	}

	.homeSocialsFrame {
		display: flex;
		width: 100%;
		height: 100%;
		align-items: center;
		justify-content: center;
		margin-inline: auto;
	}

	.homeSocials {
		display: grid;
		grid-template-columns: repeat(3, minmax(0, 1fr));
		place-content: center;
		place-items: center;
		gap: 0.75rem;
	}

	@media (prefers-reduced-motion: no-preference) {
		.homeProfileStage {
			animation: root-flip-in-x 1s both;
		}

		.homeRole {
			animation: home-role-pulse 2s cubic-bezier(0.4, 0, 0.6, 1) infinite;
		}

		.homeSocials {
			animation: root-fade-in 3s both;
		}
	}

	.homeSocialCard {
		opacity: 0.7;
		transition:
			opacity 0.5s cubic-bezier(0.4, 0, 0.2, 1),
			transform 0.5s cubic-bezier(0.4, 0, 0.2, 1),
			box-shadow 0.5s cubic-bezier(0.4, 0, 0.2, 1),
			background-color 0.5s cubic-bezier(0.4, 0, 0.2, 1);
	}

	.homeSocialCard:hover {
		z-index: 10;
		opacity: 1;
		transform: scale(1.1);
		background-color: #8881;
		box-shadow:
			0 20px 25px -5px rgb(0 0 0 / 10%),
			0 8px 10px -6px rgb(0 0 0 / 10%);
	}

	.homeSocialIcon {
		font-size: 4.5vh;
	}

	:global(html.dark) .homeRole {
		color: var(--color-text-200);
	}

	:global(html.dark) .homeSocialCard {
		opacity: 0.5;
	}

	@media (prefers-color-scheme: dark) {
		:global(html:not(.js)) .homeRole {
			color: var(--color-text-200);
		}

		:global(html:not(.js)) .homeSocialCard {
			opacity: 0.5;
		}
	}

	@media (min-width: 40rem) {
		.homeHandle {
			display: inline;
		}

		.homeSocials {
			grid-template-columns: repeat(6, minmax(0, 1fr));
		}
	}

	@media (min-width: 48rem) {
		.homeProfileStage :global(.homeProfileImage) {
			width: 16rem;
			height: 16rem;
		}
	}

	@keyframes home-role-pulse {
		50% {
			opacity: 0.5;
		}
	}

	@keyframes root-fade-in {
		from {
			opacity: 0;
		}

		to {
			opacity: 1;
		}
	}

	@keyframes root-flip-in-x {
		0% {
			opacity: 0;
			animation-timing-function: ease-in;
			transform: perspective(400px) rotateX(90deg);
		}

		40% {
			animation-timing-function: ease-in;
			transform: perspective(400px) rotateX(-20deg);
		}

		60% {
			opacity: 1;
			transform: perspective(400px) rotateX(10deg);
		}

		80% {
			transform: perspective(400px) rotateX(-5deg);
		}

		100% {
			transform: perspective(400px);
		}
	}
</style>
