...
type TweetFetchModule = {
	fetchTweet: (id: string) => Promise<TweetData | null>;
};
...
function createDependencies(server: ViteDevServer): DevRouteDependencies {
	...
	// Tweet snapshots are generated by the pre-commit hook, so uncommitted
	// articles have none. Fetch missing tweets live and cache them in memory so
	// drafts render real tweet embeds during development.
	const fetchedTweets = new Map<string, Promise<TweetData | null>>();
	const fetchMissingTweet = async (id: string): Promise<TweetData | null> => {
		let pending = fetchedTweets.get(id);
		if (pending == null) {
			pending = (
				server.ssrLoadModule('/packages/content/src/tweet-fetch.ts') as Promise<TweetFetchModule>
			).then(async (module) => {
				const tweet = await module.fetchTweet(id);
				// Drop unavailable tweets from the cache so a transient network
				// failure retries on the next page load.
				if (tweet == null) {
					fetchedTweets.delete(id);
				}
				return tweet;
			});
			fetchedTweets.set(id, pending);
		}
		return pending;
	};
