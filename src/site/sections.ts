...
type OssProjectSource = Omit<OssProject, 'link' | 'slug' | 'description'> & Partial<Pick<OssProject, 'link' | 'slug' | 'description'>>;

export async function loadOssProjects(root: string): Promise<Record<string, OssProject[]>> {
	const source = JSON.parse(await readFile(path.join(root, 'src/contents/works/oss/list.json'), 'utf8')) as Record<string, OssProjectSource[]>;
	const entries = await Promise.all(Object.entries(source).map(async ([genre, projects]) => [genre, await Promise.all(projects.map(async (project) => {
		...
		if (description == null) {
			try {
				...
				if (response.ok) {
					const data = await response.json() as { repo?: { description?: string | null } };
...
export async function loadShowcase(root: string): Promise<ShowcaseProject[]> {
	...
	return projects.sort((a, b) => Number(b.featured) - Number(a.featured) || b.pubDate.localeCompare(a.pubDate));
}
...
export async function loadPublications(root: string): Promise<Record<string, Array<{ title: string; link: string; authors: string; publisher: string }>>> {
	return JSON.parse(await readFile(path.join(root, 'src/contents/publication.json'), 'utf8')) as Record<string, Array<{ title: string; link: string; authors: string; publisher: string }>>;
}
