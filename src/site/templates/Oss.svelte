...
<div class='mt-12 grid gap-16'>
	{#each projectGroups as group (group.kind)}
		{@const groupProjects = projects.filter((project) => project.kind === group.kind)}
		{#if groupProjects.length > 0}
			<section>
				<h2 class='f-text-32-64 my-8 font-mono font-bold leading-none text-stroke-aaa text-transparent opacity-35 dark:opacity-20'>{group.title}</h2>
				<div class='grid grid-cols-1 gap-8 md:grid-cols-2'>
					{#each groupProjects as project (project.link)}
