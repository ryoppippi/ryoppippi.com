...
- Page/layout components use scoped `<style>` blocks. Post-local islands may use scoped styles and `<svelte:head>`; the upstream HTML-host renderer returns their head contributions. The current GTV chart retains ordinary CSS imports. Production page/layout CSS is emitted through upstream SSR stylesheet roots; do not restore production injected CSS or manual article CSS build inputs. Development retains injected scoped CSS until the upstream dev stylesheet resolver supports the site aliases. Format with `pnpm format` (Oxfmt through Vite+); do not add a separate Prettier configuration.
