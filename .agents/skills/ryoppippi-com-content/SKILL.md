...
- Page/layout components use scoped `<style>` blocks. Post-local islands use ordinary CSS imports because the HTML-host renderer does not return island head metadata or injected scoped styles. Format with `pnpm format` (Oxfmt through Vite+); do not add a separate Prettier configuration.
