...
- Read an existing post in the blog tree for the supported Markdown extensions and frontmatter shape.
...
- Ox Content's fetched Tweet records and self-hosted media are generated artifacts. Do not hand-edit them; run `direnv exec . pnpm build` after changing an embed and commit the generated evidence it requires.
...
- Treat the current implementation and tests as the source of truth when the accepted syntax, path resolution, or SSR boundary is unclear.
